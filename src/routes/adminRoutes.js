...
router.delete('/properties/:id', async (req, res) => {
  ...
  try {
    ...
    `;
    const deleteRes = await query(rawSql, [cleanId, rawIdNoPrefix, propIdWithPrefix]);

    // Failsafe: Direct Supabase REST DELETE request
    try {
    ...
    res.json({ success: true, message: `Property ${id} deleted successfully from database.` });
  } catch (err) {
    console.error('[Admin API] DB property delete error:', err.message);
    res.status(500).json({ success: false, message: err.message || `Failed to delete property ${id} from database.` });
  }
});
